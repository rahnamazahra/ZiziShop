...
@section('content')
    ...
    @php($product = ($products ?? collect())->first())
    @if($product)
                        ...
                        <div class="tp-product-details-action-wrapper">
                            ...
                            <div class="tp-product-details-action-item-wrapper d-sm-flex align-items-center">
                                ...
                                <div class="tp-product-details-add-to-cart mb-15 w-100">
                                    <a href="{{ route('add.to.cart', $product) }}" class="tp-product-details-add-to-cart-btn w-100 d-inline-flex align-items-center justify-content-center">افزودن به سبد خرید</a>
                                </div>
    ...
    <script>
        // پیجینیشن سمت‌کلاینت: هر تب اگر بیش از ۸ محصول داشت، صفحه‌بندی می‌شود
        (function () {
            const PER_PAGE = 8;
            document.querySelectorAll('#nav-tabContent .tab-pane').forEach(function (pane) {
                const row = pane.querySelector('.row');
                if (!row) return;
                const items = Array.from(row.querySelectorAll('.infinite-item'));
                if (items.length <= PER_PAGE) return;

                const pageCount = Math.ceil(items.length / PER_PAGE);
                const pager = document.createElement('div');
                pager.className = 'gr-pagination';

                function show(page) {
                    items.forEach(function (it, i) {
                        it.style.display = (i >= page * PER_PAGE && i < (page + 1) * PER_PAGE) ? '' : 'none';
                    });
                    pager.querySelectorAll('button').forEach(function (b, i) {
                        b.classList.toggle('is-active', i === page);
                    });
                    const top = pane.getBoundingClientRect().top + window.scrollY - 120;
                ...
                for (let p = 0; p < pageCount; p++) {
                    ...
                    b.type = 'button';
                    b.textContent = new Intl.NumberFormat('fa-IR').format(p + 1);
                    b.addEventListener('click', function () { show(p); });
                    pager.appendChild(b);
                }

                row.after(pager);
                // نمایش صفحه‌ی اول بدون اسکرول
                items.forEach((it, i) => { it.style.display = i < PER_PAGE ? '' : 'none'; });
                pager.querySelector('button').classList.add('is-active');
            });
        })();
    </script>
